...
class ConversationHandler {
  ...
  private async handleConversation(req: Request<{}, {}, OwnApiRequest>, res: Response<OwnApiResponse>): Promise<void> {
    ...
    const conversationMemoryValues = Object.values(conversationMemory);

    const stringValuesArray = conversationMemoryValues.map(memory => {
    ...
    const messages: Message[] = [
      {
        content: `You answer questions. If question is about knowledge what you know from context just answer.,
        context\`\`\
        Model knowledge: ${resultString}`,
